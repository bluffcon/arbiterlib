...
$execute if items block ~ ~ ~ container.16 * run return run function arbiterlib:craft/workbench/craft/button_vanilla/stack/add {namespace:$(namespace),item:$(item),max_stack:$(max_stack),count:$(count),translation:$(translation),model:$(model)}
