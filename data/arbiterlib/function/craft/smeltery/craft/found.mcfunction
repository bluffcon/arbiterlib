...
item modify block ~ ~ ~ container.6 {type:set_custom_data,tag:{arbiterlib:{item: "null"}}}
$item modify block ~ ~ ~ container.6 {type:set_lore, lore:[[{"translate":"ui.arbiterlib.preview_item", color:white,italic:true}], [{"translate":"ui.arbiterlib.required_heat_craft", color:white,italic:true}, {"text":"$(needed_heat)", color:gold,italic:true}, {"translate":"ui.arbiterlib.closing_bracket", color:white,italic:true}]], mode:"append"}
item modify block ~ ~ ~ container.6 {type:set_components,components:{rarity:common}}
